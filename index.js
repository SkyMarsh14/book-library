...
const kafkaOnTheShore = new Book(
  "Kafka on the Shore",
  "Haruki Murakami",
  "250",
  false
...
const closeBtn = modalBtn.addEventListener("click", () => {
  dialog.showModal();
});
...
function validateForm() {
  const constraints = {
    title: ["Title must be 6-30 charactors.", "^.{6,30}$"],
    author: [
  ...
  const titleConstraint = new RegExp(constraints.title[1], "");
  const authorConstraint = new RegExp(constraints.author[1], "");

  if (titleConstraint.test(titleField.value)) {
    titleField.setCustomValidity("");
  } else {
    titleField.setCustomValidity(constraints.title[0]);
  }

  if (authorConstraint.test(authorField.value)) {
    authorField.setCustomValidity("");
  } else {
    authorField.setCustomValidity(constraints.author[0]);
  }
};
window.onload=()=>{
    document.querySelector('#book-title').oninput=validateForm;
    document.querySelector('#book-author').oninput=validateForm;
}